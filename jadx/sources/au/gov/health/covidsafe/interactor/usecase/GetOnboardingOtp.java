...
@Metadata(
    ...
    d2 =
        {"Lau/gov/health/covidsafe/interactor/usecase/GetOnboardingOtp;",
         ...
         "awsClient", "Lau/gov/health/covidsafe/networking/service/AwsClient;",
         ...
         "TAG", "", "run", "Lau/gov/health/covidsafe/interactor/Either;",
         "Ljava/lang/Exception;", "Lkotlin/Exception;", "params",
...
public final class GetOnboardingOtp
    extends UseCase<OTPChallengeResponse, GetOtpParams> {
  ...
  public Object
  run(GetOtpParams getOtpParams,
      Continuation<? super Either<? extends Exception, OTPChallengeResponse>>
          continuation) {
    try {
      Response<OTPChallengeResponse> execute =
          this.awsClient
              .initiateAuth(new OTPChallengeRequest(
                  getOtpParams.getPhoneNumber$app_release(),
                  getOtpParams.getDeviceId$app_release(),
                  getOtpParams.getPostCode$app_release(),
                  getOtpParams.getAge$app_release(),
                  getOtpParams.getName$app_release()))
              .execute();
      if (execute.code() == 200) {
        ...
        return new Failure(new GetOnboardingOtpException.GetOtpServiceException(
            Boxing.boxInt(execute.code())));
      } else if (execute.code() == 400) {
        CentralLog.Companion.d(this.TAG, "AWSAuthInvalidNumber");
        return new Failure(
            GetOnboardingOtpException.GetOtpInvalidNumberException.INSTANCE);
      } else {
      ...
      return new Failure(new GetOnboardingOtpException.GetOtpServiceException(
          (Integer)null, 1, (DefaultConstructorMarker)null));
    }
  }
}
