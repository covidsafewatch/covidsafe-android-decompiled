...
public final class TaskRunner {
    ...
    public final List<TaskQueue> activeQueues() {
        List<TaskQueue> plus;
        synchronized (this) {
            plus = CollectionsKt.plus((Collection<? extends T>) this.busyQueues, (Iterable<? extends T>) this.readyQueues);
        }
        return plus;
    }
    ...
    private final void cancelAll() {
        for (int size = this.busyQueues.size() - 1; size >= 0; size--) {
            ((TaskQueue) this.readyQueues.get(size)).cancelAllAndDecide$okhttp();
        }
