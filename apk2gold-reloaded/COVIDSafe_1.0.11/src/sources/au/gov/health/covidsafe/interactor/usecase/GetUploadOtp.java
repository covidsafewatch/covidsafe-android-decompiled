...
import au.gov.health.covidsafe.interactor.usecase.GetUploadOtpException.GetUploadOtpServiceException;
...
public final class GetUploadOtp extends UseCase<UploadOTPResponse, String> {
    ...
    public Object run(String str, Continuation<? super Either<? extends Exception, UploadOTPResponse>> continuation) {
        ...
        try {
            ...
            sb.append("Bearer ");
            sb.append(str);
            Response execute = awsClient2.requestUploadOtp(sb.toString()).execute();
            if (execute.code() == 200) {
                CentralLog.Companion.d(this.TAG, "onCodeUpload");
                either = new Success(execute.body());
            } else {
                either = new Failure(new GetUploadOtpServiceException(Boxing.boxInt(execute.code())));
            }
            return either;
        } catch (Exception e) {
