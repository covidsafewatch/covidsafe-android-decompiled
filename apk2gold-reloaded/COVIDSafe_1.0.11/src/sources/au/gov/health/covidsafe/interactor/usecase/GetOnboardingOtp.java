...
import au.gov.health.covidsafe.interactor.usecase.GetOnboardingOtpException.GetOtpServiceException;
...
public final class GetOnboardingOtp extends UseCase<OTPChallengeResponse, GetOtpParams> {
    ...
    public Object run(GetOtpParams getOtpParams, Continuation<? super Either<? extends Exception, OTPChallengeResponse>> continuation) {
        try {
            ...
            OTPChallengeRequest oTPChallengeRequest = new OTPChallengeRequest(getOtpParams.getPhoneNumber$app_release(), getOtpParams.getDeviceId$app_release(), getOtpParams.getPostCode$app_release(), getOtpParams.getAge$app_release(), getOtpParams.getName$app_release());
            Response execute = awsClient2.initiateAuth(oTPChallengeRequest).execute();
            if (execute.code() == 200) {
                OTPChallengeResponse oTPChallengeResponse = (OTPChallengeResponse) execute.body();
                if (oTPChallengeResponse != null) {
                    Companion companion = CentralLog.Companion;
                    String str = this.TAG;
                    StringBuilder sb = new StringBuilder();
                    sb.append("onCodeSent: ");
                    OTPChallengeResponse oTPChallengeResponse2 = (OTPChallengeResponse) execute.body();
                    sb.append(oTPChallengeResponse2 != null ? oTPChallengeResponse2.getChallengeName() : null);
                    companion.d(str, sb.toString());
                    return new Success(oTPChallengeResponse);
                }
                CentralLog.Companion.d(this.TAG, "AWSAuthInvalidBody");
                return new Failure(new GetOtpServiceException(Boxing.boxInt(execute.code())));
            } else if (execute.code() == 400) {
                CentralLog.Companion.d(this.TAG, "AWSAuthInvalidNumber");
                return new Failure(GetOtpInvalidNumberException.INSTANCE);
            } else {
