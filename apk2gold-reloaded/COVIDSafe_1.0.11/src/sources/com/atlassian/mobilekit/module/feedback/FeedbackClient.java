...
class FeedbackClient implements Receiver<FeedbackConfig>, UiInfoListener {
    ...
    public final void sendFeedback(String str, final String str2) {
        Builder components = new Builder().summary(str).description(str).isCrash(false).udid(this.deviceInfo.getUdid()).uuid(this.deviceInfo.getUuid()).appName(this.deviceInfo.getAppName()).appId(this.deviceInfo.getAppPkgName()).systemName(this.deviceInfo.getSystemName()).deviceName(this.deviceInfo.getDeviceName()).language(this.deviceInfo.getLanguage()).components(Arrays.asList(this.feedbackConfig.getComponents()));
        ...
        SendFeedbackCommand sendFeedbackCommand = new SendFeedbackCommand(this.baseQueryMap, components, this.feedbackDataProvider, this.restClient, new SnackbarReceiver(this.uiInfo, this.uiNotifier, str, str2), this.uiNotifier);
        this.jobQueue.enqueue(sendFeedbackCommand);
    }
    ...
    public final void notifySendCompleted(Result result) {
        Iterator it = this.sendFeedbackListeners.iterator();
        while (it.hasNext()) {
            ((SendFeedbackListener) it.next()).onSendCompleted(result);
        }
    }
    ...
    public final void notificationStarted() {
        ...
        while (it.hasNext()) {
            ((FeedbackNotificationListener) it.next()).onNotificationStarted();
        }
    }
    ...
    public final void notificationDismissed() {
        Iterator it = this.notificationListeners.iterator();
        while (it.hasNext()) {
            ((FeedbackNotificationListener) it.next()).onNotificationDismissed();
        }
    }
